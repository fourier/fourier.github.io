...
For Dvorak layout on this keyboard I've designed the following corresponding APL layout:
...
One feature I particularly like in Dyalog IDEs (both classic and RIDE) is the toolbar with all the APL symbols. One could just point a mouse cursor on any symbol to get the tooltip with the information about this symbol. It makes it extremely convenient while studying and to just quickly recap the symbol meaning.

It could be extremely helpful to have the same feature in GNU APL, now since we have a documentation available directly from the interpreter. So [this pull request](https://github.com/lokedhs/gnu-apl-mode/pull/27) does exactly that.

It provides tooltips with documentation while mouse over the APL symbol:
![GNU APL tooltips]({{ "/assets/2018-01-20-gnu-apl-tooltips.jpg" | absolute_url }})

Additionally the behavior was changed so then the user clicks on a symbol on a popup keyboard the symbol is getting inserted into the last active buffer with GNU APL mode enabled.
